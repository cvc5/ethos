...
(step ap2 :rule check_append :args ((! S :nil) (cons c1 c2) (cons (! S :nil) c1 c2)))
(step ap3 :rule check_append :args (c1 (! S :nil) (remove cons c2 (cons c1 c2))))
...
(step co1 :rule check_concat :args ((! S :nil) (cons c1 c3) (cons c1 c3)))
(step co2 :rule check_concat :args ((cons c1 c3) (! S :nil) (cons c1 c3)))
...
(step rm4 :rule check_remove :args ((! S :nil) (cons c1 c2 c3) (cons c1 c2 c3)))
(step rm5 :rule check_remove :args (c1 (! S :nil) (! S :nil)))
...
(declare-rule check_naryElim((in S) (out S))
    :args (in out)
    :requires (((naryElim cons (! S :nil) in) out))
    :conclusion true
)
...
(step elim3 :rule check_naryElim :args ((remove cons c2 (cons (! S :nil) c2)) (! S :nil)))
(step elim4 :rule check_naryElim :args ((remove cons c2 (cons c2 (! S :nil))) (! S :nil)))
(step elim5 :rule check_naryElim :args ((remove cons c2 (cons (cons c1 c3) c2)) (cons c1 c3)))

; naryIntro
(declare-rule check_naryIntro((in S) (out S))
    :args (in out)
    :requires (((naryIntro cons (! S :nil) in) out))
    :conclusion true
)
