...
- Reference files now support `reset-assertions`, which discards the reference assertions read so far, and parse and ignore the commands `get-assertions`, `get-assignment`, `get-info`, `get-model`, `get-option`, `get-proof`, `get-unsat-assumptions`, `get-unsat-core` and `get-value`.
