...
Terms are composed of applications, built-in operators of the language (e.g. for performing computations, see [computation](#computation)), literals (see [literals](#literals)), and three categories of atomic terms (*constants*, *variables*, and *parameters*) which we will describe in the following.
...
The Eunoia language uses the SMT-LIB version 3.0 attributes `:var <symbol>` and `:implicit` in term annotations, for naming arguments of functions and specifying that they are implicit (respectively).
...
The above example declares a predicate `eq` whose first argument is a type, that is given name `T`. It then expects two terms of type `T` and returns a `Bool`. In the definition of `P`, this predicate is applied to two variables, where their type, `Int` in this case, must be explicitly provided.
...
In contrast, the above example declares a predicate `=` where the type of the arguments is implicit (this corresponds to the SMT-LIB standard definition of equality). In the definition of `P`, the type `Int` of the arguments to `=` does not need to be explicitly provided.
...
The Eunoia language supports term annotations on declared constants, which, for instance, can allow the user to treat a constant as being variadic, i.e. taking an arbitrary number of arguments. The available annotations in Eunoia for this purpose are:
- `:right-assoc` (resp. `:left-assoc`) denoting that the declared constant is right (resp. left) associative,
- `:right-assoc-nil <term>` (resp. `:left-assoc-nil <term>`) denoting that the declared constant is right (resp. left) associative with the given nil terminator given by `<term>`,
...
- `:list`, denoting that the parameter should be treated as a list when appearing as a child of an application of an associative operator.
...
Binary and hexadecimal values are implemented as a layer on top of numeral values that tracks a bit-width.
...
All rules not marked with `:ethos` are not supported by the checker and will cause the checker to terminate.
...
If the keyword is `:ethos`, what follows is the definition of a recursive program, employing a syntax given below. 
...
The provided type parameters (`<typed-param>*`) are implicit and are used to define the program's type signature and body.

The type of the program is given immediately after the type parameters list, provided as a list of argument types and a return type.
The semantics of the program is given by a non-empty list of pairs of terms, which we call its *body*.
For program `f`, this list has the form `(((f t11 ... t1n) r1) ... ((f tm1 ... tmn) rm))`
where `t11...t1n, ..., tm1...tmn` do not contain computational operators.
A (ground) term `(f s1 ... sn)` evaluates by finding the first term in the first component of a pair of `f`'s body that matches it for a substitution `S`, and returns the result of applying `S` to second component of said pair.
