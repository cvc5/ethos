...
| a definition of the **native layer**, `plugins/lean_meta/lean.eos` | `define-native-method` | one block of the layer the generated Lean is written over: the Lean it is, under `:lean-impl`. How much of the embedding the block needs in scope is read off that Lean rather than declared beside it, see `native_needs` in `tools/eoc/sem_compile.py`. It names the native the way a set does, without the `native_` the embedding gives it; whatever else its Lean defines is private to the entry and is called `impl_native_` rather than `native_` to say so |
