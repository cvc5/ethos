...
Each non-Boolean literal category must have a `declare-consts` rule before its type is needed. Type checking a literal or a builtin operator that returns that category without a rule is an error. For example, `eo::len` requires a rule for `<numeral>`, and `eo::nameof` requires a rule for `<string>`. Variable names in `eo::var`, including those generated by binder syntax, also require a rule for `<string>`. Literals normalized to another category use that category's rule.
