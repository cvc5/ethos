...
(declare-datatype sm.Term
  ...
  (sm.Const (sm.Const.id Int) (sm.Const.Type Term))  ; user constants
  (sm.Stuck)
  ; Booleans
  (sm.BoolType)
  (sm.true)
...
(assert (forall ((x1 sm.Term))
  ...
  (ite ((_ is sm.Apply) x1)
    (= ($eo_typeof x1) ($eo_typeof_apply t (sm.Apply.arg1 x1) (sm.Apply.arg2 x1)))
  (ite ((_ is sm.Var) x1)
...
(assert (forall ((x sm.Term)) ((_ is sm.Numeral) ($eo_hash x))))
(assert (forall ((x sm.Term) (y sm.Term)) (=> (= ($eo_hash x) ($eo_hash y)) (= x y))))
