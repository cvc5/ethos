...
ModelSmtNew::ModelSmtNew(State& s) : StdPlugin(s)
{
  ...
  addLitBinSym("bvadd", {kBitVec, kBitVec}, "x1", "($smt_builtin_z_add x2 x4)");
  addLitBinSym("bvmul", {kBitVec, kBitVec}, "x1", "($smt_builtin_z_mul x2 x4)");
  addLitBinSym("bvudiv",
               {kBitVec, kBitVec},
               "x1",
  ...
  addLitBinSym("bvshl",
               {kBitVec, kBitVec},
               "x1",
               "($smt_builtin_z_mul x2 ($smtx_pow2 x4))");
  addLitBinSym("bvlshr",
               {kBitVec, kBitVec},
               "x1",
  ...
  ssUgtRet << "($vsm_bool " << smtApp(">", "x2", "x4") << ")";
  addLitSym("bvugt",
            {kBitVec, kBitVec},
            kT,
  ...
  ssSgtRet << "($smtx_model_eval (or (and (not msb_s) msb_t)";
  ...
  addLitSym("zero_extend",
            ...
            smtGuard(smtApp(">=", "x1", "$smt_builtin_z_zero"),
                     "($vsm_binary ($smt_builtin_z_add x1 x2) x3)"));
  ...
  for (size_t i = 0; i < 3; i++)
  {
    ...
    if (i == 0)
    {
      ...
      ssRet << " (ite (and (not msb_s) (not msb_t)) (bvudiv s t)";
      ssRet << " (ite (and msb_s (not msb_t)) (bvneg (bvudiv (bvneg s) t))";
    ...
    else if (i == 1)
    {
      op = "bvsrem";
      ssRet << "($smtx_model_eval";
      ssRet << " (ite (and (not msb_s) (not msb_t)) (bvurem s t)";
      ssRet << " (ite (and msb_s (not msb_t)) (bvneg (bvurem (bvneg s) t))";
      ssRet << " (ite (and (not msb_s) msb_t) (bvneg (bvurem s (bvneg t)))";
      ssRet << " (bvurem (bvneg s) (bvneg t))))))))";
    }
    else
    {
      op = "bvsmod";
      ...
      ssRet << "(eo::define ((u (bvurem abs_s abs_t))) ($smtx_model_eval";
      ssRet << " (ite (= u ($eo_mk_binary x1 $smt_builtin_z_zero)) u";
      ssRet << " (ite (and (not msb_s) (not msb_t)) u";
      ssRet << " (ite (and msb_s (not msb_t)) (bvadd (bvneg u) t)";
      ssRet << " (ite (and (not msb_s) msb_t) (bvadd u t)";
      ssRet << " (bvneg u)))))))))))";
    }
  ...
  addLitSym("bvuaddo",
            {kBitVec, kBitVec},
            kBool,
            smtApp(">=", "($smt_builtin_z_add x2 x4)", "($smtx_pow2 x1)"));
  addLitSym("bvumulo",
            {kBitVec, kBitVec},
            kBool,
            smtApp(">=", "($smt_builtin_z_mul x2 x4)", "($smtx_pow2 x1)"));
  for (size_t i = 0; i < 2; i++)
  {
    ...
    ssRet << " " << smtApp("<", "sret", "($smt_builtin_z_neg p2wm1)");
    ssRet << ")))";
  ...
  addLitSym(
      "@bv", {kInt, kInt}, kT, "($eo_mk_binary x2 x1)");
  addEunoiaReduceSym("@bit", {kInt, kBitVec}, "(extract x1 x1 x2)");
...
void ModelSmtNew::finalizeDecl(const std::string& name, const Expr& e)
{
  ...
  std::map<std::string, std::vector<Kind>>::iterator itsty = d_symTypes.find(name);
                                    ...
                                    std::ostream& os)
{
  ...
  {
    os << "($vsm_" << d_kindToEoPrefix[k] << " " << term << ")";
  }
