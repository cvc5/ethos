...
(define-fun $eo_eq ((x1 eo.Term) (x2 eo.Term)) eo.Term
  (ite (or (= x1 eo.Stuck) (= x2 eo.Stuck))
    eo.Stuck
  (ite (= x2 x1)
    (eo.SmtTerm (eo.SmtTerm sm.True))
  (ite true
    (eo.SmtTerm (eo.SmtTerm sm.False))
    eo.Stuck)))
)

; define $eo_is_eq
(define-fun $eo_is_eq ((t eo.Term) (s eo.Term)) eo.Term ($eo_ite ($eo_and ($eo_is_ok t) ($eo_is_ok s)) ($eo_eq s t) (eo.SmtTerm (eo.SmtTerm sm.False))))

; define $eo_is_z
(define-fun $eo_is_z ((x eo.Term)) eo.Term ($eo_ite ($eo_and ($eo_is_ok ($eo_to_z x)) ($eo_is_ok x)) ($eo_eq x ($eo_to_z x)) (eo.SmtTerm (eo.SmtTerm sm.False))))

; define $eo_is_q
(define-fun $eo_is_q ((x eo.Term)) eo.Term ($eo_ite ($eo_and ($eo_is_ok ($eo_to_q x)) ($eo_is_ok x)) ($eo_eq x ($eo_to_q x)) (eo.SmtTerm (eo.SmtTerm sm.False))))

; define $eo_is_bin
(define-fun $eo_is_bin ((x eo.Term)) eo.Term ($eo_ite ($eo_and ($eo_is_ok ($eo_to_bin ($eo_len x) x)) ($eo_is_ok x)) ($eo_eq x ($eo_to_bin ($eo_len x) x)) (eo.SmtTerm (eo.SmtTerm sm.False))))

; define $eo_is_str
(define-fun $eo_is_str ((x eo.Term)) eo.Term ($eo_ite ($eo_and ($eo_is_ok ($eo_to_str x)) ($eo_is_ok x)) ($eo_eq x ($eo_to_str x)) (eo.SmtTerm (eo.SmtTerm sm.False))))

; define $eo_is_bool
(define-fun $eo_is_bool ((x eo.Term)) eo.Term ($eo_ite ($eo_ite ($eo_and ($eo_is_ok x) (eo.SmtTerm (eo.SmtTerm sm.True))) ($eo_eq (eo.SmtTerm (eo.SmtTerm sm.True)) x) (eo.SmtTerm (eo.SmtTerm sm.False))) (eo.SmtTerm (eo.SmtTerm sm.True)) ($eo_ite ($eo_and ($eo_is_ok x) (eo.SmtTerm (eo.SmtTerm sm.True))) ($eo_eq (eo.SmtTerm (eo.SmtTerm sm.False)) x) (eo.SmtTerm (eo.SmtTerm sm.False)))))
...
(define-fun $eo_typeof_main ((x1 eo.Term)) eo.Term
  ...
  (ite (= x1 (eo.SmtTerm sm.BoolType))
    (eo.SmtTerm sm.Type)
  (ite (= x1 (eo.SmtTerm (eo.SmtTerm sm.True)))
    (eo.SmtTerm sm.BoolType)
  (ite (= x1 (eo.SmtTerm (eo.SmtTerm sm.False)))
    (eo.SmtTerm sm.BoolType)
  ...
  (ite (and ((_ is eo.SmtTerm) x1) ((_ is sm.Apply) (eo.SmtTerm.arg1 x1)) (= (sm.Apply.arg1 (eo.SmtTerm.arg1 x1)) sm.=))
    ($eo_typeof_= ($eo_typeof (sm.Apply.arg2 (eo.SmtTerm.arg1 x1))))
...
(assert (! (forall ((x1 eo.Term))
  ...
  (ite (= x1 eo.Stuck)
    eo.Stuck
  (ite true
    ($eo_requires (eo.SmtTerm (eo.SmtTerm sm.True)) (eo.SmtTerm (eo.SmtTerm sm.False)) (eo.SmtTerm (eo.SmtTerm sm.True)))
    eo.Stuck))))
...
(assert (! (forall ((x1 eo.Term))
  ...
  (ite (and ((_ is eo.SmtTerm) x1) ((_ is sm.Apply) (eo.SmtTerm.arg1 x1)) ((_ is sm.Apply) (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))) (= (sm.Apply.arg1 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))) sm.$eo_Var))
    (sm.Apply.arg2 (eo.SmtTerm.arg1 x1))
  (ite true
    ($eo_ite ($eo_ite ($eo_ite ($eo_and ($eo_is_ok x1) (eo.SmtTerm (eo.SmtTerm sm.True))) ($eo_eq (eo.SmtTerm (eo.SmtTerm sm.True)) x1) (eo.SmtTerm (eo.SmtTerm sm.False))) (eo.SmtTerm (eo.SmtTerm sm.True)) ($eo_ite ($eo_and ($eo_is_ok x1) (eo.SmtTerm (eo.SmtTerm sm.True))) ($eo_eq (eo.SmtTerm (eo.SmtTerm sm.False)) x1) (eo.SmtTerm (eo.SmtTerm sm.False)))) (eo.SmtTerm sm.BoolType) ($eo_ite ($eo_ite ($eo_and ($eo_is_ok ($eo_to_z x1)) ($eo_is_ok x1)) ($eo_eq x1 ($eo_to_z x1)) (eo.SmtTerm (eo.SmtTerm sm.False))) (eo.SmtTerm sm.Int) ($eo_ite ($eo_ite ($eo_and ($eo_is_ok ($eo_to_q x1)) ($eo_is_ok x1)) ($eo_eq x1 ($eo_to_q x1)) (eo.SmtTerm (eo.SmtTerm sm.False))) (eo.SmtTerm sm.Real) ($eo_ite ($eo_ite ($eo_and ($eo_is_ok ($eo_to_str x1)) ($eo_is_ok x1)) ($eo_eq x1 ($eo_to_str x1)) (eo.SmtTerm (eo.SmtTerm sm.False))) (eo.SmtTerm (sm.Apply sm.Seq sm.Char)) ($eo_ite ($eo_ite ($eo_and ($eo_is_ok ($eo_to_bin ($eo_len x1) x1)) ($eo_is_ok x1)) ($eo_eq x1 ($eo_to_bin ($eo_len x1) x1)) (eo.SmtTerm (eo.SmtTerm sm.False))) (eo.SmtTerm sm.$eo_undef_type) ($eo_typeof_main x1))))))
    eo.Stuck)))))
...
(define-fun $eovc_symm ((x1 eo.Term)) eo.Term
  (ite (= x1 eo.Stuck)
    eo.Stuck
  (ite true
    ($eo_requires ($eo_model_sat x1) (eo.SmtTerm (eo.SmtTerm sm.True)) ($eo_requires ($eo_model_sat ($eor_symm x1)) (eo.SmtTerm (eo.SmtTerm sm.False)) (eo.SmtTerm (eo.SmtTerm sm.True))))
    eo.Stuck))
)
...
(assert (! (forall ((x1 eo.Term) (x2 eo.Term))
  ...
  (ite (= x1 (eo.SmtTerm sm.Real))
    ($eo_ite ($eo_eq ($eo_zmod ($eo_zdiv ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 0))) ($eo_ite ($eo_eq ($eo_zmod ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 0))) ($smt_rat_to_num ($smt_rat_enum_rec ($eo_zdiv ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))) ($eo_Apply ($eo_Apply eo.$eo_List_cons ($eo_Apply ($eo_Apply eo.@smt_ratPair (eo.SmtTerm (sm.Numeral 1))) (eo.SmtTerm (sm.Numeral 1)))) ($eo_Apply ($eo_Apply eo.$eo_List_cons eo.$eo_List_nil) eo.$eo_List_nil)))) ((eo.SmtTerm sm.$smt_unknown_type_enum) (eo.SmtTerm sm.Real) ($eo_zdiv ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))))) ($eo_neg ($eo_ite ($eo_eq ($eo_zmod ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 0))) ($smt_rat_to_num ($smt_rat_enum_rec ($eo_zdiv ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2))) ($eo_Apply ($eo_Apply eo.$eo_List_cons ($eo_Apply ($eo_Apply eo.@smt_ratPair (eo.SmtTerm (sm.Numeral 1))) (eo.SmtTerm (sm.Numeral 1)))) ($eo_Apply ($eo_Apply eo.$eo_List_cons eo.$eo_List_nil) eo.$eo_List_nil)))) ((eo.SmtTerm sm.$smt_unknown_type_enum) (eo.SmtTerm sm.Real) ($eo_zdiv ($eo_zdiv x2 (eo.SmtTerm (sm.Numeral 2))) (eo.SmtTerm (sm.Numeral 2)))))))
  (ite true
...
(assert (! (forall ((x1 eo.Term) (x2 eo.Term) (x3 eo.Term))
  ...
  (ite (or (= x1 eo.Stuck) (= x2 eo.Stuck) (= x3 eo.Stuck))
    eo.Stuck
  (ite true
    ($eo_ite ($eo_is_ok ($smt_type_enum x1 x3)) ($eo_ite ($eo_eq ($smt_type_enum x1 x3) x2) (eo.SmtTerm (eo.SmtTerm sm.True)) ($smt_type_enum_contains_rec x1 x3 ($eo_add x3 (eo.SmtTerm (sm.Numeral 1))))) (eo.SmtTerm (eo.SmtTerm sm.False)))
    eo.Stuck))))
...
(assert (! (forall ((x1 eo.Term))
  ...
  (ite (= x1 eo.Stuck)
    eo.Stuck
  (ite ((_ is eo.Apply) x1)
    ($eo_ite ($smt_is_value ($smt_typeof (eo.Apply.arg2 x1)) (eo.Apply.arg2 x1)) ($smt_dt_is_value (eo.Apply.arg1 x1)) (eo.SmtTerm (eo.SmtTerm sm.False)))
  (ite true
...
(assert (! (forall ((x1 eo.Term) (x2 eo.Term))
  ...
  (ite (= x1 (eo.SmtTerm sm.$smt_unknown_type))
    (eo.SmtTerm (eo.SmtTerm sm.False))
  (ite (and (= x1 (eo.SmtTerm sm.BoolType)) (= x2 (eo.SmtTerm (eo.SmtTerm sm.True))))
    (eo.SmtTerm (eo.SmtTerm sm.True))
  (ite (and (= x1 (eo.SmtTerm sm.BoolType)) (= x2 (eo.SmtTerm (eo.SmtTerm sm.False))))
    (eo.SmtTerm (eo.SmtTerm sm.True))
  ...
  (ite (and ((_ is eo.Apply) x1) ((_ is eo.Apply) (eo.Apply.arg1 x1)) (= (eo.Apply.arg1 (eo.Apply.arg1 x1)) eo.@Map))
    ($smt_map_is_value x2)
  (ite true
    ($eo_ite ($eo_is_ok ($eo_dt_constructors x1)) ($smt_dt_is_value x2) ($smt_type_enum_contains_rec x1 x2 (eo.SmtTerm (sm.Numeral 0))))
    eo.Stuck)))))))))))
...
(assert (! (forall ((x1 eo.Term))
  ...
  (ite (and ((_ is eo.SmtTerm) x1) ((_ is sm.Apply) (eo.SmtTerm.arg1 x1)) (= (sm.Apply.arg1 (eo.SmtTerm.arg1 x1)) sm.not))
    ($eo_mk_bool (not ($smt_from_eo_bool ($smt_model_eval (sm.Apply.arg2 (eo.SmtTerm.arg1 x1))))))
  (ite (and ((_ is eo.SmtTerm) x1) ((_ is sm.Apply) (eo.SmtTerm.arg1 x1)) ((_ is sm.Apply) (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))) (= (sm.Apply.arg1 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))) sm.and))
    ($eo_mk_bool (and ($smt_from_eo_bool ($smt_model_eval (sm.Apply.arg2 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))))) ($smt_from_eo_bool ($smt_model_eval (sm.Apply.arg2 (eo.SmtTerm.arg1 x1))))))
  (ite (and ((_ is eo.SmtTerm) x1) ((_ is sm.Apply) (eo.SmtTerm.arg1 x1)) ((_ is sm.Apply) (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))) (= (sm.Apply.arg1 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))) sm.=))
    ($eo_ite ($eo_and ($smt_is_value ($smt_typeof (sm.Apply.arg2 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1)))) ($smt_model_eval (sm.Apply.arg2 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1))))) ($smt_is_value ($smt_typeof (sm.Apply.arg2 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1)))) ($smt_model_eval (sm.Apply.arg2 (eo.SmtTerm.arg1 x1))))) ($eo_mk_bool (= ($smt_model_eval (sm.Apply.arg2 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1)))) ($smt_model_eval (sm.Apply.arg2 (eo.SmtTerm.arg1 x1))))) (eo.SmtTerm (sm.Apply (sm.Apply sm.= (sm.Apply.arg2 (sm.Apply.arg1 (eo.SmtTerm.arg1 x1)))) (sm.Apply.arg2 (eo.SmtTerm.arg1 x1)))))
  (ite ((_ is (eo.SmtTerm sm.$smt_Const)) x1)
    ($eo_ite ($smt_model_eval ($eo_ite ($eo_eq ((eo.SmtTerm sm.$smt_Const).arg1 x1) (eo.SmtTerm (sm.Numeral 0))) (eo.SmtTerm (eo.SmtTerm sm.True)) ($eo_ite ($eo_is_ok ($eo_const_predicate ((eo.SmtTerm sm.$smt_Const).arg1 x1) ((eo.SmtTerm sm.$smt_Const).arg2 x1) ((eo.SmtTerm sm.$smt_Const).arg3 x1))) ($eo_const_predicate ((eo.SmtTerm sm.$smt_Const).arg1 x1) ((eo.SmtTerm sm.$smt_Const).arg2 x1) ((eo.SmtTerm sm.$smt_Const).arg3 x1)) (eo.SmtTerm (eo.SmtTerm sm.True))))) ($smt_model_lookup ((eo.SmtTerm sm.$smt_Const).arg1 x1) ((eo.SmtTerm sm.$smt_Const).arg2 x1) ((eo.SmtTerm sm.$smt_Const).arg3 x1)) ((eo.SmtTerm sm.$smt_Const) ((eo.SmtTerm sm.$smt_Const).arg1 x1) ((eo.SmtTerm sm.$smt_Const).arg2 x1) ((eo.SmtTerm sm.$smt_Const).arg3 x1)))
  (ite ((_ is eo.Apply) x1)
    ($eo_ite ($eo_is_ok ($eo_model_eval (($smt_model_eval (eo.Apply.arg1 x1)) ($smt_model_eval (eo.Apply.arg2 x1))))) ($eo_model_eval (($smt_model_eval (eo.Apply.arg1 x1)) ($smt_model_eval (eo.Apply.arg2 x1)))) (($smt_model_eval (eo.Apply.arg1 x1)) ($smt_model_eval (eo.Apply.arg2 x1))))
  (ite true
...
(assert (! (forall ((x1 eo.Term))
  ...
  (ite true
    ($eo_ite ($eo_is_ok ($eo_typeof x1)) ($eo_typeof x1) (eo.SmtTerm sm.$smt_unknown_type))
    eo.Stuck)))))))
